...
public class UserService {
     ...
     *   <li>없으면, 신규 생성 경로에서만 name blank 검증 → blank면 {@link GoogleAuthException}</li>
     *   <li>del 무관 연결 이력이 있으면(=탈퇴 이력) → {@link AccountLoginBlockedException} (재가입/재활성 안 함)</li>
     *   <li>이력도 없으면 → {@link UserRegistrationService#register}로 신규 생성(isNewUser=true)</li>
     * </ol>
     *
    ...
    public UserFindOrCreateResult findOrCreate(GoogleUserInfo googleUserInfo) {
        ...
        try {
            ...
            // 다른 요청이 먼저 같은 sub로 생성을 커밋함 → 그 사용자를 재조회해 반환.
            OAuthConnection winner =
                    oauthRepository.findByProviderAndProviderUserIdAndDelFalse(GOOGLE, googleUserInfo.sub())
                            .orElseThrow(() -> new IllegalStateException(
                                    "동시 최초 로그인 경합 후에도 OAuth 연결을 찾지 못했습니다: sub=" + googleUserInfo.sub(),
                                    race));
