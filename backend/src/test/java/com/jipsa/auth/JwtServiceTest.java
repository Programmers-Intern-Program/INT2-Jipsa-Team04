...
class JwtServiceTest {
    ...
    void 변조된_토큰은_거부된다() {
        ...
        assertThat(jwtService.validateAndGetUserId(token + "tampered")).isEmpty();
    }

    @Test
    void 다른_키로_서명검증하면_거부된다() {
        String token = new JwtService(SECRET, ACCESS_MS).generateToken(42L);
        JwtService otherKey = new JwtService("another-secret-0123456789-abcdefghij-xyz", ACCESS_MS);

        assertThat(otherKey.validateAndGetUserId(token)).isEmpty();
    }
    ...
    void 만료된_토큰은_거부된다() {
        ...
        String expired = expiring.generateToken(42L);

        assertThat(expiring.validateAndGetUserId(expired)).isEmpty();
    }
    ...
    void 형식이_잘못된_토큰은_거부된다() {
        ...
        Optional<Long> result = jwtService.validateAndGetUserId("not-a-jwt");
