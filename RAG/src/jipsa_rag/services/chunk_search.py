"""사용자 질의를 임베딩하고 관련 청크 검색 결과를 API 응답으로 조정한다."""
...
class ChunkSearchRepository(Protocol):
    """검색 서비스가 필요로 하는 사용자 범위 청크 검색 계약."""
    ...
    async def search(
        ...
        *,
        ...
        """사용자 범위와 최소 점수를 적용하여 관련 청크를 조회한다."""
...
class ChunkSearchService:
    ...
    async def search(
        ...
        """요청의 top_k와 최소 유사도를 적용하여 검증된 청크를 반환한다."""
        ...
        # 결과 개수, 사용자 범위, 점수 임계값과 정렬 순서를 다시 확인한다.
        #
        # 이 방어 검증은 향후 Repository 구현체가 교체되거나 테스트 대역이
        # 잘못된 값을 반환하더라도 다른 사용자의 청크가 API 응답으로 전달되는
        # 상황을 차단한다.
...
def _validate_search_hits(
    *,
    ...
    """Repository 결과가 API 요청 계약을 위반하지 않는지 검증한다."""
    ...
    for hit in hits:
        if hit.users_idx != request.user_idx:
            raise InvalidVectorSearchResultError("search_user_scope_contract_violation")

        if request.score_threshold is not None and hit.score < request.score_threshold:
