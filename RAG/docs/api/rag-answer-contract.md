...
> 계약 버전: `1.2.1`  
> 적용 범위: Local RAG의 검색, 부분 답변, 최종 종합, 인용 및 출처 응답
...
혼합 문서 답변 기능의 코드와 문서가 서로 다른 상태로 배포되지 않도록 다음
회귀 계약을 유지한다.

- README에 PDF, DOCX, PPTX, XLSX, TXT와 OCR 지원 범위가 모두 존재한다.
- README에 사용자·활성 색인·선택 문서 Qdrant 필터가 존재한다.
- 두 계약 문서가 `cited_source_ids`, 본문 최초 등장 순서와 최종 `sources` 축소를
  동일하게 설명한다.
- 형식별 locator 필드와 OCR `image_ordinal`이 문서에서 제거되지 않는다.
- 유효한 부분 근거가 없을 때 최종 Claude 호출을 생략한다는 설명이 유지된다.
- 과거의 PDF 전용 또는 OCR 미지원 문구가 다시 들어오지 않는다.

이 문서 변경을 포함한 commit은 `scripts/verify-rag-quality.ps1`을 기준으로 Ruff,
Mypy strict와 전체 Pytest를 통과해야 한다. 실제 외부 서비스 E2E는 opt-in 상태를
명시하며, 실행하지 않은 항목을 통과한 것으로 기록하지 않는다.
